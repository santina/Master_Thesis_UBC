---
title: "Analysis Top20 and more info on singular values"
author: "Santina Lin"
...
output: 
  html_document: 
    ...
    toc: yes
---
(this is copied pasted from "analysis_top10.Rmd". The only difference is that the data source is swapped out. The graphs from this one are the ones to go into the thesis)
...
term_frequency <- prepare_data("data/term_freq_precision_top20.result") 
term_binary <- prepare_data("data/term_binary_precision_top20.result")
tf_idf <- prepare_data("data/tfidf_precision_top20.result")
```
...
all_result_top10 <- rbind(tf_idf, term_binary, term_frequency)
all_result_top10 <- all_result[all_result$nsv <=1000, ]  # Cut off unimportant curve 
```
...
# singular values inpsection

## singluar value and error estimates
Sanity checking singular value and and their errors. 

## singular value variance/frobenius
