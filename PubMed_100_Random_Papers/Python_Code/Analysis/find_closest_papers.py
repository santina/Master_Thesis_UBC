...
def findClosestRows(matrix, rowNum, nNeighbors, distfunc, nsv):
	...
	index = 0
	normalizedM = normalizeMatrix(matrix, nsv)
	for row in normalizedM:

		if distfunc == euclidean: 
			dist = distfunc(normalizedM[rowNum][:nsv], row[:nsv])
		elif distfunc == cosine:
			dist = 1 - np.dot(normalizedM[rowNum][:nsv], row[:nsv])
	...
	with open(outfile, 'w') as out:
		for i in range(0, matrix.shape[0]):
			...
			if pubMedID in categories:  # Only calculate for the row of the category 
				result = findClosestRows(matrix, i, npapers_h[pubMedID], cosine, nsv)
				writeToFile(pubMedID, result, out, paperID_h)

def writeEuclideanResults(matrix, outfile, categories, paperID_h, npapers_h):
	''' Go through the entire matrix and find closest rows to each row
	...
	with open(outfile, 'w') as out:
		for i in range(0, matrix.shape[0]):
			pubMedID = paperID_h[i]
			if pubMedID in categories:  # Only calculate for the row of the category 
				result = findClosestRows(matrix, i, npapers_h[pubMedID], euclidean)
				writeToFile(pubMedID, result, out, paperID_h)
...
def main():  # many inputs! 
	...
	matrix = readMatrix(args.m, args.nrow, args.nsv)
	svals = getSVals(args.s, args.nsv)
	matrix = scaleMatrix(matrix, svals)
	outputFile = args.out + '/{0}_{1}.neighbors'
	print "Time took to build the matrix:" + str(timeit.default_timer() - t)

	# try each different nsv, differ by args.step in each step 
	for nsv in range(args.start, args.nsv+1, args.step): 
		
		t = timeit.default_timer()
		outfile = outputFile.format('cosine', str(nsv))
		writeCosineResults(matrix, outfile, categories, paperID_h, npapers_h, nsv)
